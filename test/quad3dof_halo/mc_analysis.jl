...
path_mc  = "/home/samuelbuckner/HALO_ROS/halo_mc_results/test_20250114_003953" 

# Parse mc data
data = Dict()
for (_,_,files) in walkdir(path_mc)
    for file in files
        file_ = replace(file, ".jld2" => "")
        ...
        data_ = load(joinpath(path_mc,file))
        append!(data[spec], [data_["summary"]])
    end
end
