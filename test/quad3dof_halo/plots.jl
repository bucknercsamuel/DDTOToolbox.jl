...
function plot_mc_statistics(solution_set; interactive=true, groupings::Vector = [], label="")
    ...
    ax = Axis(f[1,idx], ylabel="Cumulative energy [N^(3/2)]"; defaults...)
    add_plot_entries(ax, solution_set, "cum_energy"; colors=colors, saturate_zero=true, groupings=groupings, outlier_threshold=1e6)
    push!(axes, ax)
    idx += 1
    types *= "cum_energy_"
