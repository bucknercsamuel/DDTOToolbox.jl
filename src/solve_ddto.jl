...
function solve_ddto_pdg(lander::Lander, costs_optimal_0::CVector)::Vector{DDTOSolution}
    ...
    for k = 1:(n_targs_total-1)
        ...
        ddto_branch_sols[k] = solve_bisection_qcvx_ddto(lander_, costs_optimal_0, cost_dd_sum)
...
function solve_bisection_qcvx_ddto(lander::Lander, costs_optimal::CVector, cost_dd::CReal)::DDTOSolution
...
function solve_feasible_ddto(lander::Lander, τ::Int, costs_optimal::CVector, cost_dd::CReal)::Tuple{DDTOSolution, MOI.TerminationStatusCode}
    ...
    for j = 1:n
        ...
        # @constraint(mdl, T[3,1,j]       == -dot(e_z, lander.g)/lander.mass) # Vertical orientation constraint
