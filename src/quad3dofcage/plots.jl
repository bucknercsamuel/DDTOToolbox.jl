...
function plot_parametric_optimal_trajectories(params::Params, solutions::Vector{ProcessedBranchSolution}, simulations::Vector{ProcessedBranchSolution})
...
function plot_parametric_ddto_trajectories(params::Params, solutions::Array{ProcessedBranchSolution}, simulations::Array{ProcessedBranchSolution})
...
function plot_states(params::Params, sols_ddto::Array{BranchSolution})
    ...
    for j in 1:params.n_targs
        data_targ = vcat(
            sols_ddto[j].sol.r,
            sols_ddto[j].sol.v,
            hcat(sols_ddto[j].sol.T / params.mass, [0;0;0]),
        )
        push!(data, data_targ)
    end
